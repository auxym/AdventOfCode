# Runs in about 5 minutes, using 2 GB ram, on my i5-4310U laptop.
...
type GameState = object
  tiles: array[2..6, array[2..12, Amphipod]]
...
func isRoom(v: Vector): bool = v.y in 3..6 and v.x in {4, 6, 8, 10}
...
func finalState(): GameState =
  for y in 3..6:
    result[(4, y)]  = Amphipod.A
    result[(6, y)]  = Amphipod.B
    result[(8, y)]  = Amphipod.C
    result[(10, y)]  = Amphipod.D
...
func isLegalRoomMove(st: GameState, v: Vector, kind: Amphipod): bool =
  ...
  if v.x != xkind: return false
  for y in 3..6:
    if not (st.isEmpty((xkind, y)) or st[(xkind, y)] == kind):
      return false
  return true
